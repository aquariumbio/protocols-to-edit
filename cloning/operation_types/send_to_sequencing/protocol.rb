...
class Protocol
  ...
  def main
    ...
    operations.make
    operations.each do |op|
      raise "The output collection for operation #{op.id} could not be made successfully" if op.output(SEQ_RESULT).item.nil?
    end
  ...
  def display_genewiz_instructions(operations)
    show_return = show do
      ...
      check "Go the <a href='https://www.genewiz.com' target='_blank'>GENEWIZ website</a>. Log in with lab account (Username: #{GENEWIZ_USER}, password is #{GENEWIZ_PASS})."
      check "Click on Plasmid, enter DNA type from the table below, select Premix, Same Day, enter the number of samples, then click Apply."
      check 'Enter DNA Name, DNA length, and My Primer Name according to the following table, choose DNA Type to be Plasmid.'
